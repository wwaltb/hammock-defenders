...
function Moth.new(self)
	...
	self.target = { x = 58, y = 77 }

	-- Base orbit parameters
	self.angularSpeed = 1.5 + math.random() * 1.5
	self.wobble = 0.25
...
function Moth.update(self, dt)
	...
	if self.stateTimer <= 0 then
		...
		if self.state == "rest" then
			...
			self.state = "burst"
			self.stateTimer = 0.15 + math.random() * 0.6
			self.arcAngle = (math.random() - 0.5) * 0.8 -- arc direction
			self.angularSpeed = 2.7 + math.random() * 1.6
		...
		elseif self.state == "burst" then
			...
			self.state = "rest"
			self.stateTimer = 0.5 + math.random() * 0.8
			self.angularSpeed = 0.6 + math.random() * 0.5
		elseif self.state == "prepare" then
			self.state = "attack"
			self.stateTimer = 0.1 + math.random() * 0.2
		...
		elseif self.state == "recover" then
			self.state = "burst"
			self.stateTimer = 0.1 + math.random() * 0.6
			self.arcAngle = (math.random() - 0.5) * 0.8 -- arc direction
			self.angularSpeed = 2.5 + math.random() * 1.5
			if math.random() > 0.7 then
				self.burstDir = 1
			else
				self.burstDir = -1
	...
	if self.state == "prepare" and self.stateTimer < 0.65 and self.stateTimer > 0.5 then
		self.glowing = true
	elseif self.state == "attack" then
		self.glowing = true
	else
